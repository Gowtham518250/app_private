...
try {
  ...
  static Future<void> markSaleAsSynced(String saleId) async {
    ...
    if (normalized.isEmpty) {
      throw StateError('LOCAL_SYNC_ACK_FAILED:empty_sale_id');
    }
    final marked = await _markSaleAsSynced(normalized);
    if (!marked) {
      // The durable outbox item must remain queued when the local ACK could
      // not be persisted. Throwing here lets sync_service keep the item for a
      // safe retry instead of deleting a business record from the queue.
      throw StateError('LOCAL_SYNC_ACK_FAILED:$normalized');
    }
  }
